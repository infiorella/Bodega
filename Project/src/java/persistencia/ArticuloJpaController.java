...
public class ArticuloJpaController implements Serializable {
    ...
    public void create(Articulo articulo) throws PreexistingEntityException, Exception {
        ...
        if (articulo.getKardexList() == null) {
            articulo.setKardexList(new ArrayList<Kardex>());
        }
        ...
        try {
            ...
            for (Kardex kardexListKardexToAttach : articulo.getKardexList()) {
                kardexListKardexToAttach = em.getReference(kardexListKardexToAttach.getClass(), kardexListKardexToAttach.getNum());
                attachedKardexList.add(kardexListKardexToAttach);
            }
            articulo.setKardexList(attachedKardexList);
            ...
            for (Kardex kardexListKardex : articulo.getKardexList()) {
                Articulo oldCodOfKardexListKardex = kardexListKardex.getCod();
                kardexListKardex.setCod(articulo);
                kardexListKardex = em.merge(kardexListKardex);
                if (oldCodOfKardexListKardex != null) {
                    oldCodOfKardexListKardex.getKardexList().remove(kardexListKardex);
                    oldCodOfKardexListKardex = em.merge(oldCodOfKardexListKardex);
                }
            }
    ...
    public void edit(Articulo articulo) throws IllegalOrphanException, NonexistentEntityException, Exception {
        ...
        try {
            ...
            List<Kardex> kardexListOld = persistentArticulo.getKardexList();
            List<Kardex> kardexListNew = articulo.getKardexList();
            ...
            for (Kardex kardexListOldKardex : kardexListOld) {
                if (!kardexListNew.contains(kardexListOldKardex)) {
                    ...
                    illegalOrphanMessages.add("You must retain Kardex " + kardexListOldKardex + " since its cod field is not nullable.");
                }
            }
            ...
            for (Kardex kardexListNewKardexToAttach : kardexListNew) {
                kardexListNewKardexToAttach = em.getReference(kardexListNewKardexToAttach.getClass(), kardexListNewKardexToAttach.getNum());
                attachedKardexListNew.add(kardexListNewKardexToAttach);
            }
            kardexListNew = attachedKardexListNew;
            articulo.setKardexList(kardexListNew);
            ...
            for (Kardex kardexListNewKardex : kardexListNew) {
                if (!kardexListOld.contains(kardexListNewKardex)) {
                    Articulo oldCodOfKardexListNewKardex = kardexListNewKardex.getCod();
                    kardexListNewKardex.setCod(articulo);
                    kardexListNewKardex = em.merge(kardexListNewKardex);
                    if (oldCodOfKardexListNewKardex != null && !oldCodOfKardexListNewKardex.equals(articulo)) {
                        oldCodOfKardexListNewKardex.getKardexList().remove(kardexListNewKardex);
                        oldCodOfKardexListNewKardex = em.merge(oldCodOfKardexListNewKardex);
                    }
                }
            }
    ...
    public void destroy(String id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            List<Kardex> kardexListOrphanCheck = articulo.getKardexList();
            for (Kardex kardexListOrphanCheckKardex : kardexListOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Articulo (" + articulo + ") cannot be destroyed since the Kardex " + kardexListOrphanCheckKardex + " in its kardexList field has a non-nullable cod field.");
            }
